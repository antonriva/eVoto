...
export const useElectoresActions = (setElectores) => {
  ...
  const eliminarElector = useCallback(async (idElector) => {
    try {
      const res = await fetch(`${apiBase}/elector/${idElector}`, { method: "DELETE" });

      if (res.ok) {
        setElectores(prev => prev.filter(p => p.idElector !== idElector));
        return { success: true, message: "Elector eliminado exitosamente." };
      } else {
        const msg = await res.text();
        if (res.status === 404) return { success: false, error: `No existe el elector con ID ${id}.` };
  ...
  }, [apiBase, setElectores]);

  const actualizarElector = useCallback(async ({ idElector, formData }) => {
    try {
  ...
  return { fetchDomicilios, eliminarElector, actualizarElector, registrarPersona };
};
